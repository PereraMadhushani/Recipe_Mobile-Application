...
const SearchScreen = () => {
  ...
  const performSerach =async (query) => {
    //If no search query
    if(!query.trim()){
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      try {
        const results = await performSerach('');
        setRecipes(results);
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const handleSearch = async () => {
      setLoading(true);
      try {
        const results = await performSerach(debouncedSearchQuery);
        setRecipes(results);
      } catch (error) {
  ...
  return(
    <View style={searchStyles.emptyState}>
      ...
      <Text style={searchStyles.emptyDescription}>
        Try adjusting your search or try diffrent keywords
      </Text>
    </View>
  )
}